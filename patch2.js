...
async function extractAllFieldsWithAI({ sourceType = 'manual_html', url = '', html = '' }) {
  ...
  const pageHtml = String(html || '').trim();
...
\${truncatedHtml}\`;
  ...
  try {
    ...
    if (parsed) {
      ...
      pageTitle = typeof parsed.page_title === 'string' ? parsed.page_title.trim() : pageTitle;
      aiUsed = true;
    }
  } catch (err) {
    console.warn('[SCRAPER] AI extraction failed:', err.message);
...
const exportStart = content.lastIndexOf('module.exports = {');
if (exportStart === -1) {
  console.error('Could not find module.exports');
  process.exit(1);
}

const beforeExport = content.slice(0, exportStart);
const exportBlock = content.slice(exportStart);

const newExportBlock = exportBlock.replace('};', '  ,extractAllFieldsWithAI\n};');

fs.writeFileSync(file, beforeExport + '\n' + toAppend + '\n' + newExportBlock);
